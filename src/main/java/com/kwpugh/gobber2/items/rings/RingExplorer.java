...
public class RingExplorer extends Item
{
		...
		if (!world.isClient && !player.isSneaking())
		{
			if (world instanceof ServerWorld)
			{
				...
				for(int i = 1; i < 6; i++)
				{
					...
					// Use startingPos x and z for starting point of search
												...
												(y-2) > 60)))
						{
							...
							if (chunk.getBlockState(legPos).getMaterial().equals(Material.AIR))
							{
								...
								if (chunk.getBlockState(headPos).getMaterial().equals(Material.AIR))
								{
									...
									serverPlayer.sendMessage((new TranslatableText("item.gobber2.gobber2_ring_explorer.tip4")), true);   //enjoy...
									return TypedActionResult.success(stack);
								}
							}
						}
